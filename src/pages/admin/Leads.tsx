...
export default function AdminLeads() {
  ...
  const { data: leads, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Transform nested arrays returned by Supabase into single objects
      const transformed = data?.map((lead: any) => {
        ...
        // brand itself can be returned as an array (brand:brands(name) => [{ name }])
  ...
  const updateLeadStatusMutation = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: string }) => {
      const { data, error } = await supabase.from("leads").update({ lead_status: status }).eq("id", id).select();
  ...
  const updateLeadNotesMutation = useMutation({
    mutationFn: async ({ id, notes }: { id: string; notes: string }) => {
      const { data, error } = await supabase.from("leads").update({ lead_notes: notes }).eq("id", id).select();
      if (error) throw error;
      return data[0];
    },
    onSuccess: () => {
  ...
  const completedRevenue =
    filteredLeads
      ?.filter((l) => l.lead_status === "completed" && l.final_price !== null)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1">Customers who verified phone and saw price but didn't book pickup</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-emerald-700">₹{completedRevenue.toLocaleString("en-IN")}</div>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-sm text-slate-400 mt-1">Leads will appear here when customers verify phone but don't book pickup</p>
            </div>
          ) : (
      ...
      <Dialog open={detailsDialogOpen} onOpenChange={setDetailsDialogOpen}>
        <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Customer's device selection and answers</DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={notesDialogOpen} onOpenChange={setNotesDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>Internal notes for follow-up</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <Textarea value={noteText} onChange={(e) => setNoteText(e.target.value)} placeholder="Add follow-up notes, customer preferences, best time to call, etc..." className="min-h-[150px]" rows={8} />
            <p className="text-xs text-slate-500">{noteText.length} characters</p>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => { setNotesDialogOpen(false); setNoteText(""); }}>Cancel</Button>
            <Button onClick={handleSaveNotes} disabled={updateLeadNotesMutation.isPending}><Save className="mr-2 h-4 w-4" />{updateLeadNotesMutation.isPending ? "Saving..." : "Save Notes"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
