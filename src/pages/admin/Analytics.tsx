...
import { startOfDay, endOfDay, startOfMonth, format, subDays, differenceInDays } from 'date-fns';
...
export default function AdminAnalytics() {
  ...
  const { data: analytics, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      (citiesData.data as PickupRequestCity[] | undefined)?.forEach(item => {
        let cityName = 'Unknown';
        if (Array.isArray(item.city)) {
          cityName = item.city.length > 0 && item.city[0]?.name ? item.city[0].name : 'Unknown';
        } else if (item.city && typeof item.city === 'object' && 'name' in item.city) {
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {[...Array(8)].map((_, i) => (
            <Skeleton key={i} className="h-32 bg-gray-100" />
          ))}
        </div>
      </div>
    );
              ...
              <Button
                ...
                className={
                  ...
                  format(dateRange.to, 'yyyy-MM-dd') === format(filter.to, 'yyyy-MM-dd')
                    ...
                    : 'gap-2 border-gray-200 text-gray-700 hover:bg-gray-100'
                }
              >
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card className="bg-white border-gray-100 shadow-sm hover:shadow-md transition-all">
          ...
          <CardContent>
            ...
            <div className="flex items-center gap-1 text-xs mt-1">
              ...
              <span className={analytics && analytics.revenueGrowth >= 0 ? 'text-emerald-600' : 'text-red-600'}>
        ...
        <Card className="bg-white border-gray-100 shadow-sm hover:shadow-md transition-all">
          ...
          <CardContent>
            ...
            <div className="flex items-center gap-1 text-xs mt-1">
              ...
              <span className={analytics && analytics.ordersGrowth >= 0 ? 'text-emerald-600' : 'text-red-600'}>
        ...
        <Card className="bg-white border-gray-100 shadow-sm hover:shadow-md transition-all">
          ...
          <CardContent>
            ...
            <p className="text-xs text-gray-500 mt-1">Per completed pickup</p>
          </CardContent>
        </Card>
            ...
            {analytics?.dailyStats && analytics.dailyStats.length > 0 ? (
              <div className="space-y-3 max-h-96 overflow-y-auto">
                {analytics.dailyStats.map((day) => (
                  <div key={day.date} className="flex items-center justify-between border-b border-gray-100 pb-3 last:border-0 hover:bg-gray-50/50 p-2 rounded transition-colors">
                    <div>
            ...
            {analytics?.topCities && analytics.topCities.length > 0 ? (
              <div className="space-y-3">
                {analytics.topCities.map((city, index) => (
                  <div key={city.name} className="flex items-center justify-between hover:bg-gray-50/50 p-2 rounded transition-colors">
